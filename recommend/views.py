...
from .models import Food, Myfoodrating, MyfoodList
...
#for food
def food(request):
    movies = Food.objects.all()
    query = request.GET.get('q')

    if query:
        movies = Food.objects.filter(Q(title__icontains=query)).distinct()
        return render(request, 'recommendfood/list.html', {'movies': movies})
...
#for food
def detail2(request, movie_id):
    if not request.user.is_authenticated:
        return redirect("login")
    if not request.user.is_active:
        raise Http404
    movies = get_object_or_404(Food, id=movie_id)
    movie = Food.objects.get(id=movie_id)
    
    temp = list(MyfoodList.objects.all().values().filter(movie_id=movie_id,user=request.user))
    if temp:
        update = temp[0]['watch']
    else:
        update = False
    if request.method == "POST":
        ...
        if 'watch' in request.POST:
            ...
            if watch_flag == 'on':
                ...
                update = False
            if MyfoodList.objects.all().values().filter(movie_id=movie_id,user=request.user):
                MyfoodList.objects.all().values().filter(movie_id=movie_id,user=request.user).update(watch=update)
            else:
                q=MyfoodList(user=request.user,movie=movie,watch=update)
                q.save()
            if update:
                messages.success(request, "Dish added to your list!")
            else:
                messages.success(request, "Dish removed from your list!")

            
        # For rating
        else:
            rate = request.POST['rating']
            if Myfoodrating.objects.all().values().filter(movie_id=movie_id,user=request.user):
                Myfoodrating.objects.all().values().filter(movie_id=movie_id,user=request.user).update(rating=rate)
            else:
                q=Myfoodrating(user=request.user,movie=movie,rating=rate)
                q.save()

            messages.success(request, "Rating has been submitted!")

        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    out = list(Myfoodrating.objects.filter(user=request.user.id).values())
    ...
    context = {'movies': movies,'movie_rating':movie_rating,'rate_flag':rate_flag,'update':update}
    return render(request, 'recommendfood/detail.html', context)
...
def watch2(request):
    ...
    movies = Food.objects.filter(myfoodlist__watch=True,myfoodlist__user=request.user)
    query = request.GET.get('q')

    if query:
        movies = Food.objects.filter(Q(title__icontains=query)).distinct()
        return render(request, 'recommendfood/watch.html', {'movies': movies})
...
def recommend2(request):
    ...
    movie_rating=pd.DataFrame(list(Myfoodrating.objects.all().values()))

    new_user=movie_rating.user_id.unique().shape[0]
    current_user_id= request.user.id
	# if new user not rated any movie
    if current_user_id>new_user:
        movie=Food.objects.get(id=6)
        q=Myfoodrating(user=request.user,movie=movie,rating=0)
        q.save()
    ...
    user = pd.DataFrame(list(Myfoodrating.objects.filter(user=request.user).values())).drop(['user_id','id'],axis=1)
    user_filtered = [tuple(x) for x in user.values]
    movie_id_watched = [each[0] for each in user_filtered]
    ...
    movies_id_recommend = [each for each in movies_id if each not in movie_id_watched]
    preserved = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(movies_id_recommend)])
    movie_list=list(Food.objects.filter(id__in = movies_id_recommend).order_by(preserved)[:10])

    context = {'movie_list': movie_list}
    return render(request, 'recommendfood/recommend.html', context)
